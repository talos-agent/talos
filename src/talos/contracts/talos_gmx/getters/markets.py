...
class Markets(BaseModel):
    _info: dict[HexAddress, dict[str, str]] = PrivateAttr(default_factory=dict)

    async def load_info(self) -> dict[HexAddress, dict[str, Any]]:
        if not self._info:
            self._info = await self._process_markets()
        return self._info

    def get_index_token_address(self, market_key: HexAddress) -> str:
    ...
    def get_decimal_factor(self, market_key: HexAddress, long: bool = False, short: bool = False) -> int:
        if long:
    ...
    def is_synthetic(self, market_key: HexAddress) -> bool:
        return self._info[market_key]["market_metadata"]["synthetic"]

    def get_available_markets(self) -> dict[HexAddress, dict[str, str]]:
        """
    ...
    async def _get_available_markets_raw(self) -> list[MarketProps]:
        ...
        return await synthetics_reader.get_markets(
            GetMarketsParams(data_store=DATASTORE_ADDRESS, start_index=uint256(0), end_index=uint256(50))
        ).get()

    async def _process_markets(self) -> dict[HexAddress, dict[str, str]]:
        """
        ...
        checks = await asyncio.gather(
            *[self._check_if_index_token_in_signed_prices_api(raw_market.index_token) for raw_market in raw_markets]
        )
        ...
        for raw_market, check in zip(raw_markets, checks):
            ...
            try:
                ...
                if raw_market.market_token == "0x0Cf1fb4d1FF67A3D8Ca92c9d6643F8F9be8e03E5":
                    ...
                    decoded_markets[raw_market.market_token]["index_token_address"] = (
